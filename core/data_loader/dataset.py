...
class PersonReIDSamples:
    ...
    def _relabels_c(self, samples, label_index,dataset='market',s_t='market_duke',is_target=False):
        '''
        Assign 2K classification labels of source domain and target domain according to the camera
        '''
        # reorder
...
class Samples4Market(PersonReIDSamples):
    ...
    Market Dataset
    '''
    def __init__(self, market_path, is_target=False,train_st='',relabel=True, combineall=False,target_modify=False):
        ...
        if target_modify:
            train_path = os.path.join(self.market_path, 'bounding_box_train_s1/')
        else:
        ...
        # load
        train = self._load_images_path(train_path,make_c_net_id=True,is_target=is_target)
        ...
        if self.relabel:
            ...
            train = self._relabels_c(train, 4,s_t=train_st,is_target=is_target,)
...
class Samples4Duke(PersonReIDSamples):
    ...
    Duke dataset
    '''
    def __init__(self, market_path,is_test=False,is_target=False,train_st='', relabel=True, combineall=False,target_modify=False):
        ...
        # load
        if not is_test:
            train = self._load_images_path(train_path,self.dataset,make_c_net_id=True,is_target=is_target)
        query = self._load_images_path(query_path)
        gallery = self._load_images_path(gallery_path)
        if self.combineall:
        ...
        if not is_test:
            if self.relabel:
                train = self._relabels(train, 1)
                train = self._relabels_c(train, 4,'duke',train_st,is_target)
        if not is_test:
            self.train, self.query, self.gallery = train, query, gallery
            self._show_info(train, query, gallery)
        else:
...
class Samples4MSMT17(PersonReIDSamples):
    ...
    load MSMT17 dataset
    '''
    def __init__(self, path,is_target=False,train_st='', relabel=True, combineall=False,target_modify=False):

        if target_modify:
            list_train_path = os.path.join(path, 'modify/list_train.txt')
            list_val_path = os.path.join(path, 'modify/list_val.txt')
            list_query_path = os.path.join(path, 'modify/list_query.txt')
            list_gallery_path = os.path.join(path, 'modify/list_gallery.txt')
            train = self._load_list(os.path.join(path, 'test/'), list_train_path,make_c_net_id=True,is_target=is_target)
            val = self._load_list(os.path.join(path, 'test/'), list_val_path,make_c_net_id=True,is_target=is_target)
            query = self._load_list(os.path.join(path, 'train/'), list_query_path)
            gallery = self._load_list(os.path.join(path, 'train/'), list_gallery_path)
            ...
            train = self._load_list(os.path.join(path, 'train/'), list_train_path,make_c_net_id=True,is_target=is_target)
            val = self._load_list(os.path.join(path, 'train/'), list_val_path,make_c_net_id=True,is_target=is_target)
        ...
        if relabel:
            train = self._relabels(train, 1)
            train = self._relabels_c(train, 4,'msmt',train_st,is_target=is_target)
        self.train, self.query, self.gallery = train, query, gallery
        self._show_info(train, query, gallery)

    def _load_list(self, dir_path, list_path,make_c_net_id=False,is_target=False):
        ...
        for img_idx, img_info in enumerate(lines):
            ...
            if make_c_net_id:
                ...
                t_s = [[camid,camid],[camid,camid],[camid,camid],[camid,camid],[camid,camid],[camid,camid],[camid,camid],[camid,camid]]
                data.append([img_path, pid, camid,domain_label,c_s,t_s])
            else:
...
class PersonReIDDataSet:
    ...
    def _loader(self, img_path):
        ...
        # transform = A.ChannelShuffle(0.5)
