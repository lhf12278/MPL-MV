...
class PersonReIDSamples:
    ...
    def _relabels_c(self, samples, label_index,dataset='market',is_target=False):
        '''
        reorder labels
        map labels [1, 3, 5, 7] to [0,1,2,3]
        '''
        ...
        if dataset=='market' and is_target==False:
            ...
            for sample in samples:
                ...
                if (sample[2]== 1 or sample[2]==6 or sample[2]==7 or sample[2]==8):
                    sample[label_index][1][0] = 1
                    sample[label_index][1][1] = 0
                elif(sample[2]== 2 or sample[2]==3 or sample[2]==4 or sample[2]==5):
                    sample[label_index][1][0] = 0
                    sample[label_index][1][1] = 1

                if (sample[2]== 2 or sample[2]==5 or sample[2]==7 or sample[2]==8):
                    sample[label_index][2][0] = 1
                    sample[label_index][2][1] = 0
                elif(sample[2]== 1 or sample[2]==6 or sample[2]==3 or sample[2]==4):
                    sample[label_index][2][0] = 0
                    sample[label_index][2][1] = 1

                if (sample[2]== 3 or sample[2]==5 or sample[2]==6 or sample[2]==8):
                    sample[label_index][3][0] = 1
                    sample[label_index][3][1] = 0
                elif(sample[2]== 7 or sample[2]==1 or sample[2]==2 or sample[2]==4):
                    sample[label_index][3][0] = 0
                    sample[label_index][3][1] = 1
        ...
        for file_name in files_name:
            if '.jpg' in file_name:
                ...
                if make_c_net_id:
                    ...
                    c_s = [[identi_id,identi_id],[identi_id,identi_id],[identi_id,identi_id],[identi_id,identi_id]]
                    t_s = [[camera_id,camera_id],[camera_id,camera_id],[camera_id,camera_id],[camera_id,camera_id]]
...
class Samples4Market(PersonReIDSamples):
    ...
    Market Dataset
    '''
    def __init__(self, market_path, is_target=False,relabel=True, combineall=False):
        ...
        if self.relabel:
            ...
            # train = self._relabels(train, 3)
            # train = self._relabels(train, 4)
            train = self._relabels_c(train, 4,is_target=is_target)
...
class Samples4Duke(PersonReIDSamples):
    ...
    Duke dataset
    '''
    def __init__(self, market_path,is_test=False,is_target=False, relabel=True, combineall=False):
        ...
        if not is_test:
            if self.relabel:
                ...
                # train = self._relabels(train, 3)
                # train = self._relabels(train, 4)
                train = self._relabels_c(train, 4,'duke',is_target)
        if not is_test:
            self.train, self.query, self.gallery = train, query, gallery
            self._show_info(train, query, gallery)
        else:
