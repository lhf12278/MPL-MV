...
def main(config):
	...
	if config.mode == 'train':  # train mode
		...
		model_list = os.listdir(config.model_path)# test mode
		for model_name in model_list:
			a_model_path = os.path.join(config.model_path,model_name)
			print(a_model_path)
			base.resume_from_model(a_model_path)
...
if __name__ == '__main__':
	...
	parser.add_argument('--train_dataset', nargs='+', type=str, default=['market_duke'], help='market/duke/msmt/njust_win/njust_spr/njust_both. support multi-dataset such as [market, duke]')
	parser.add_argument('--test_dataset', type=str, default='duke', help='market, duke, msmt, njust_win/njust_spr/njust_both, wildtrack')
	parser.add_argument('--t_dataset', type=str, default='duke',
						help='market, duke, msmt, njust_win/njust_spr/njust_both, wildtrack')  ###目标域
	...
	# train configuration
	parser.add_argument('--steps', type=int, default=800, help='1 epoch include many steps')

	parser.add_argument('--milestones', nargs='+', type=int, default=[40, 70], help='milestones for the learning rate decay')
	parser.add_argument('--m_2k_milestones', nargs='+', type=int, default=[40, 70],help='milestones for the learning rate decay')
	parser.add_argument('--d_2k_milestones', nargs='+', type=int, default=[40, 70],help='milestones for the learning rate decay')
	parser.add_argument('--D_domainn_milestones', nargs='+', type=int, default=[40, 70],help='milestones for the learning rate decay')

	parser.add_argument('--base_learning_rate', type=float, default=0.00035)
	parser.add_argument('--m_2k_learn_rate', type=float, default=0.00035)
	parser.add_argument('--d_2k_learn_rate', type=float, default=0.00035)
	parser.add_argument('--d_domain_learn_rate', type=float, default=0.00035)
	...
	# test configuration model_path
	parser.add_argument('--model_path', type=str, default='model/', help='')
	parser.add_argument('--resume_test_model', type=str, default='model/model_119.pkl', help='')
